pub mod headwear_lotus {
    ...
    pub fn get_lotus(
        ...
        (
            ...
            get_headwear_color(lotus_color),
        )
    }

    fn get_lotus_style(lotus_color: LotusColors) -> ByteArray {
        let (light_color, base_color) = get_lotus_color(lotus_color);
        format!(
            r#"<style>.headwearColorL {{ fill:{light_color} }}.headwearColorB {{ fill:{base_color} }}.hwStr1 {{ stroke: #000; stroke-linecap: round; stroke-linejoin: round; }}</style>{}"#,
            "svgPath"
        )
    }

    fn get_lotus_color(lotus_color: LotusColors) -> (ByteArray, ByteArray) {
        match lotus_color {
            LotusColors::GREEN => (LensColors::LIGHT_GREEN, LensColors::BASE_GREEN),
            LotusColors::PINK => (LensColors::LIGHT_PINK, LensColors::BASE_PINK),
            LotusColors::PURPLE => (LensColors::LIGHT_PURPLE, LensColors::BASE_PURPLE),
            LotusColors::BLUE => (LensColors::LIGHT_BLUE, LensColors::BASE_BLUE),
            LotusColors::GOLD => (LensColors::LIGHT_GOLD, LensColors::BASE_GOLD),
        }
    }

    fn get_headwear_color(lotus_color: LotusColors) -> HeadwearColors {
        match lotus_color {
            LotusColors::GREEN => HeadwearColors::GREEN,
            LotusColors::PINK => HeadwearColors::PINK,
            LotusColors::PURPLE => HeadwearColors::PURPLE,
            LotusColors::BLUE => HeadwearColors::BLUE,
            LotusColors::GOLD => HeadwearColors::GOLD,
        }
    }
}