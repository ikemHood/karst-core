pub mod headwear_hat {
    ...
    pub fn get_hat(hat_color: HatColors) -> (ByteArray, HeadwearVariants, HeadwearColors) {
        (
            format!(
                r#"<svg xmlns="http://www.w3.org/2000/svg" width="210" height="335" fill="none">{}</svg>"#,
                get_hat_style(hat_color),
            ...
            get_headwear_color(hat_color),
        )
    }

    fn get_hat_style(hat_color: HatColors) -> ByteArray {
        let (hat_color1, hat_color2) = get_hat_color(hat_color);
        format!(
            r#"<style>.hatColor1 {{ fill: {} }} .hatColor2 {{ fill: {} }} .hatLine {{ stroke: black; stroke-linecap: round; stroke-linejoin: round }}</style>svgPath"#,
            hat_color1, hat_color2
        )
    }

    fn get_hat_color(hat_color: HatColors) -> (ByteArray, ByteArray) {
        match hat_color {
            HatColors::GREEN => (LensColors::DARK_GREEN, LensColors::LIGHT_GREEN),
            HatColors::PINK => (LensColors::DARK_PINK, LensColors::LIGHT_PINK),
            HatColors::BLUE => (LensColors::DARK_BLUE, LensColors::LIGHT_BLUE),
            HatColors::PURPLE => (LensColors::DARK_PURPLE, LensColors::LIGHT_PURPLE),
            HatColors::GOLD => (LensColors::BASE_GOLD, LensColors::LIGHT_GOLD),
        }
    }

    fn get_headwear_color(hat_color: HatColors) -> HeadwearColors {
        match hat_color {
            HatColors::GREEN => HeadwearColors::GREEN,
            HatColors::PINK => HeadwearColors::PINK,
            HatColors::BLUE => HeadwearColors::BLUE,
            HatColors::PURPLE => HeadwearColors::PURPLE,
            HatColors::GOLD => HeadwearColors::GOLD,
        }
    }
}