    ...
    pub fn get_mushroom(
        ...
        (
            ...
            get_headwear_color(mushroom_color),
        )
    }

    fn get_mushroom_style(mushroom_color: MushroomColors) -> ByteArray {
        let (primary_color, secondary_color) = get_mushroom_color(mushroom_color);
        let (sparkles_fill, sparkles_stroke) = get_mushroom_sparkles_colors(mushroom_color);
        format!(
            r#"<style>.mushroomColor1 {{ fill:{} }}.mushroomColor2 {{ fill:{} }}.mushroomSparkles {{fill:{}; stroke:{}; stroke-linecap: round; stroke-linejoin: round;}}</style>{}"#,
            primary_color, secondary_color, sparkles_fill, sparkles_stroke,
            "svgPath"
        )
    }

    fn get_mushroom_sparkles_colors(
        mushroom_color: MushroomColors,
    ) -> (ByteArray, ByteArray) {
        if mushroom_color == MushroomColors::GOLD {
            (LensColors::WHITE, LensColors::DARK_GOLD)
        } else {
            (LensColors::WHITE, LensColors::BLACK)
        }
    }

    fn get_mushroom_color(
        mushroom_color: MushroomColors,
    ) -> (ByteArray, ByteArray) {
        match mushroom_color {
            MushroomColors::GREEN => (LensColors::LIGHT_GREEN, LensColors::BASE_GREEN),
            MushroomColors::PURPLE => (LensColors::LIGHT_PURPLE, LensColors::BASE_PURPLE),
            MushroomColors::BLUE => (LensColors::LIGHT_BLUE, LensColors::BASE_BLUE),
            MushroomColors::PINK => (LensColors::LIGHT_PINK, LensColors::BASE_PINK),
            MushroomColors::GOLD => (LensColors::LIGHT_GOLD, LensColors::BASE_GOLD),
        }
    }

    fn get_headwear_color(mushroom_color: MushroomColors) -> HeadwearColors {
        match mushroom_color {
            MushroomColors::GREEN => HeadwearColors::GREEN,
            MushroomColors::PURPLE => HeadwearColors::PURPLE,
            MushroomColors::BLUE => HeadwearColors::BLUE,
            MushroomColors::PINK => HeadwearColors::PINK,
